...
/// [ExpandableBottomSheetCustom] is a BottomSheet with a draggable height like the
...
/// );
///
class ExpandableBottomSheetCustom extends StatefulWidget {
  /// [expandableContent] is the widget which you can hide and show by dragging.
  /// It has to be a widget with a constant height. It is required for the [ExpandableBottomSheetCustom].
  final Widget expandableContent;

  /// [background] is the widget behind the [expandableContent] which holds
  /// usually the content of your page. It is required for the [ExpandableBottomSheetCustom].
  ...
  /// Creates the [ExpandableBottomSheetCustom].
  ///
  /// [persistentContentHeight] has to be greater 0.
  final Function(double) callbackPersistentContentHeight;

  const ExpandableBottomSheetCustom({
    required Key key,
    required this.expandableContent,
    required this.background,
    required this.persistentHeader,
    required this.persistentFooter,
    ...
    required this.onIsExtendedCallback,
    required this.onIsContractedCallback,
    this.enableToggle = false,
    required this.callbackPersistentContentHeight,
  })  : assert(persistentContentHeight >= 0),
        super(key: key);

  @override
  ExpandableBottomSheetCustomState createState() => ExpandableBottomSheetCustomState();
}

class ExpandableBottomSheetCustomState extends State<ExpandableBottomSheetCustom> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: 300), // Đảm bảo bạn cung cấp một duration
    );
    _controller.addStatusListener(_handleAnimationStatusUpdate);

    // Sử dụng hàm ẩn danh để gọi _afterUpdateWidgetBuild với giá trị isFirstBuild là true
        ...
        Expanded(
          child: Stack(
            ...
            children: <Widget>[
              ...
              AnimatedBuilder(
                animation: _controller, // Ensure _controller is an initialized AnimationController
                builder: (context, child) {
                  if (_controller.isAnimating) {
                    // Correctly reference _controller and ensure other variables are accessible
                  ...
                  return Positioned(
                    ...
                    right: 0.0,
                    left: 0.0,
                    child: child!, // `child` is non-nullable, ensure it's used correctly
                  );
                },
                child: GestureDetector(
                  onTap: _toggle, // Ensure _toggle is defined and correctly handles tap events
                  onVerticalDragDown: _dragDown, // Ensure _dragDown is defined
                  onVerticalDragUpdate: _dragUpdate, // Ensure _dragUpdate is defined
                  onVerticalDragEnd: _dragEnd, // Ensure _dragEnd is defined
                  child: Column(
  ...
  void _dragUpdate(DragUpdateDetails details) {
    // setState(() {
    //   widget.persistentContentHeight  = 0;
    // });
    print("drag update");
    widget.callbackPersistentContentHeight(widget.persistentContentHeight);
  ...
  void _dragEnd(DragEndDetails details) {
    ...
    double average = (_maxOffset + _minOffset) / 2;
    setState(() {
      if (_positionOffset > average) {
        _positionOffset = _maxOffset;
      } else {
        _positionOffset = _minOffset;
      }
